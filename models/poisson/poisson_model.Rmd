...
```{r}
gam_full <- gam(los ~ s(charges) + s(age) + diagnosis + sex + drg + charges.na + diagnosis:drg + 
                  diagnosis:charges + sex:charges + sex:age + sex:charges.na + 
                  drg:charges + drg:age + drg:charges.na + charges:age, family = 'poisson', data = ami_train)
summary(gam_full)
```

```{r}
save(gam_full, file = 'poisson_gam.rds')
```
...
ami_train <- ami_train %>% 
  mutate(
    charges_8 = (charges - mean(charges)) ^ 8,
    age_8 = (age - mean(age)) ^ 8
  )

# update formula to be glm
newvars <- c("charges_8", "age_8")
gam_glm_form <- attr(gam_full$terms, "term.labels") %>% 
  paste(., collapse = "+") %>% 
  paste(., paste(newvars, collapse ="+"), sep = "+") %>% 
  paste("los ~", .)

# Refit a glm with GAM variables
poisson_gam_glm <- glm(formula(gam_glm_form), data = ami_train, family='poisson')
anova(best_int, poisson_gam_glm , test = "Chisq")
```
...
```{r}
#check training performance of gam
gam_pred <- predict(gam_full, ami_train, type = 'response')
rmse_vec(ami_train$los, gam_pred)
```
